...
| `right` (*input_string*, *input_integer*) → *output_string*| Returns the last *input_integer* characters in the *input_string*. If *input_integer* is negative, the first *input_integer* characters are removed from *output_string*. | `right('risingwave', 4)` → `wave` <br /> `right('risingwave', 4)` → `gwave`|
