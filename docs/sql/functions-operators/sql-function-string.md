...
|regexp_matches(*input_string*, *pattern*) → *set_of_matched_string*[] | Returns a set of string arrays of captured substring(s) resulting from matching a POSIX regular expression pattern to a string. Returns all matches by default.| `REGEXP_MATCHES('foobarbequebazilbarfbonk', '(b[^b]+)(b[^b]+)')` → `{bar,beque} {bazil,barf}`|
