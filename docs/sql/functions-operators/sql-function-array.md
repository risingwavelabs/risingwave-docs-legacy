...
|array_remove(*any_array*, *any_compatible*) → *output_array* | Returns an array will all occurrences of *any_compatible* removed. Multidimensional arrays are also supported. | `array_remove(array[array[1],array[2],array[3]], array[2])` → `{{1},{3}}`|
