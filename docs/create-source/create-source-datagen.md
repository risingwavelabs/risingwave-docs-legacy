...
|`max_past_mode`|Specify the baseline timestamp. <br/> The range for generated timestamps is [basetime - `max_past` , basetime]|`absolute` — Baseline is set to the execution time of the generator.<br />`relative` —  Baseline is the current system time.|False<br/>Default: `absolute`|
|`basetime`|If set, the generator will use the specified time as the basetime. If unspecified, the generator execution time is set as the basetime.|A [date and time string](https://docs.rs/chrono/latest/chrono/struct.DateTime.html#method.parse_from_rfc3339)<br/>Example: `2023-04-01T16:39:57-08:00`|False<br/>Default: generator execution time|
