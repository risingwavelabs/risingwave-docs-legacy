...
|*location*| Web location of the schema file in `http://...`, `https://...`, or `S3://...` format. For Protobuf data, you must specify either a schema location or a schema registry but not both. For Avro data, it's not supported.|
|*schema.registry*| Confluent Schema Registry URL. Example: `http://127.0.0.1:8081`. For Protobuf data, you must specify either a schema location or a Confluent Schema Registry but not both; for Avro data, you must specify a Confluent Schema Registry.|
